...
// Format percentage: ±###.## (without % symbol), "-" for 0 count
const formatPct = (val: number, count?: number) => {
  if (count === 0) return '-';
  const sign = val >= 0 ? '+' : '';
...
const getSegmentClasses = (segments: Record<string, SegmentStats>, timeSegments: TimeSegment[]): Record<string, string> => {
  const values = timeSegments.map(seg => ({
    key: seg.key,
    pct: segments[seg.key]?.pct ?? 0,
  }));

  const positives = values.filter(v => v.pct > 0);
  const negatives = values.filter(v => v.pct < 0);

  const WHITE = 'text-foreground';
  const GREEN = 'text-emerald-400';
  const RED = 'text-rose-400';

  const maxPositive = positives.length > 0 ? Math.max(...positives.map(v => v.pct)) : null;
  const minNegative = negatives.length > 0 ? Math.min(...negatives.map(v => v.pct)) : null;

  const result: Record<string, string> = {};
  values.forEach(v => {
    if (v.pct > 0 && v.pct === maxPositive) {
      result[v.key] = GREEN;
    } else if (v.pct < 0 && v.pct === minNegative) {
      result[v.key] = RED;
    } else {
      result[v.key] = WHITE;
    }
...
const winrateClass = (rate: number, count?: number) =>
  count === 0 ? 'text-muted-foreground' : rate > 50 ? 'text-emerald-400' : rate < 50 ? 'text-rose-400' : 'text-foreground';

const formatWinRate = (rate: number, count?: number) =>
  count === 0 ? '-' : `${Math.round(rate)}%`;

export default function Analysis11SegPctPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${API_BASE}/dev/analysis-11seg-pct/summary`);
    ...
    <main className="relative min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 leading-[1.8] tracking-[0.02em] font-sans">
        ...
        <header className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 mb-4 pb-3 border-b border-border/30">
          <div>
            <h1 className="text-xl font-bold text-foreground">11時間区分分析（変化率）</h1>
            <p className="text-muted-foreground text-sm">
              {dateRange.from} ~ {dateRange.to} ({dateRange.tradingDays}営業日) | ショート基準 | 単位：%
            </p>
          </div>
          <DevNavLinks />
        </header>
                        ...
                        return (
                          <div key={seg.key} className="text-right min-w-[70px]">
                            ...
                            <div className={`text-xl font-bold tabular-nums ${classes[seg.key]}`}>
                              {formatPct(stats?.pct ?? 0, stats?.count)}
                            </div>
                          </div>
                        );
                      });
                                ...
                                {timeSegments.map(seg => {
                                  ...
                                  return (
                                    <td key={seg.key} className="text-right px-2 py-2.5">
                                      ...
                                      <div className={`text-xs ${winrateClass(stats?.winRate ?? 0, stats?.count)}`}>
                                        {formatWinRate(stats?.winRate ?? 0, stats?.count)}
                                      </div>
                                    </td>
                                  );
                                })}
