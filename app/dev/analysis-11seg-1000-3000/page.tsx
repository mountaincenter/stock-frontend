...
interface ApiResponse {
  generatedAt: string;
  fourSegments: TimeSegment[];
  elevenSegments: TimeSegment[];
  priceRanges: string[];
  dateRange: {
    from: string;
    to: string;
...
export default function Analysis11Seg1000To3000Page() {
  ...
  const fetchData = async (excludeEx: boolean) => {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE}/dev/analysis-11seg-1000-3000/summary?exclude_extreme=${excludeEx}`);
  ...
  const handleExcludeExtremeChange = (exclude: boolean) => {
    setExcludeExtreme(exclude);
    fetchData(exclude);
  };

  // 現在のセグメント定義を取得
  const getCurrentSegments = (): TimeSegment[] => {
    if (!data) return [];
    return segmentMode === '4seg' ? data.fourSegments : data.elevenSegments;
  };
  ...
  const getSegmentData = (
    ...
    return segmentMode === '4seg' ? segments4 : segments11;
  };
  ...
  const timeSegments = getCurrentSegments();
  ...
  return (
    <main className="relative min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 leading-[1.8] tracking-[0.02em] font-sans">
        ...
        <header className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 mb-4 pb-3 border-b border-border/30">
          <div>
            <h1 className="text-xl font-bold text-foreground">
              1,000~3,000円 特化分析
              {excludeExtreme && <span className="ml-2 text-amber-400 text-base font-normal">（異常日除外中）</span>}
            </h1>
          ...
          <div className="flex items-center gap-4">
            ...
            <label className="flex items-center gap-2 text-sm text-muted-foreground cursor-pointer">
              <input
                ...
                className="w-4 h-4 rounded border-border/40 bg-muted/30 accent-primary"
              />
              <span>1/13,15除外</span>
            </label>
            <DevNavLinks />
        ...
        {(() => {
          const overallSegs = getSegmentData(
          ...
          const actualSegs = segmentMode === '4seg'
            ? (overallFilter === 'ex0' ? overall.segments4.ex0 : overall.segments4.all)
            : (overallFilter === 'ex0' ? overall.segments11.ex0 : overall.segments11.all);
            ...
            <div className="mb-6 relative overflow-hidden rounded-xl border border-border/40 bg-gradient-to-br from-card/50 via-card/80 to-card/50 p-4 shadow-lg shadow-black/5 backdrop-blur-xl">
              ...
              <div className="relative">
                ...
                <div className="overflow-x-auto mb-3 pb-3 border-b border-border/30">
                  <div className="flex justify-end gap-5 min-w-[600px]">
                    {(() => {
                      const classes = getSegmentClasses(actualSegs, timeSegments);
                      return timeSegments.map(seg => {
                        const stats = actualSegs[seg.key];
                        return (
        ...
        {weekdays.map((wd, idx) => {
          ...
          const seidoSegs = getSegmentData(wd.seido.segments4, wd.seido.segments11);
          const ichiSegs = getSegmentData(
                ...
                <div className="relative overflow-hidden rounded-xl border border-border/40 bg-gradient-to-br from-card/50 via-card/80 to-card/50 p-4 shadow-lg shadow-black/5 backdrop-blur-xl">
                  ...
                  <div className="relative">
                    ...
                    <div className="overflow-x-auto mb-3 pb-3 border-b border-border/30">
                      <div className="flex justify-end gap-5 min-w-[600px]">
                        {(() => {
                          const classes = getSegmentClasses(seidoSegs, timeSegments);
                          return timeSegments.map(seg => {
                      ...
                      <table className="w-full text-sm min-w-[600px]">
                        ...
                        <tbody>
                          {wd.seido.priceRanges.map(pr => {
                            const prSegs = getSegmentData(pr.segments4, pr.segments11);
                            const classes = getSegmentClasses(prSegs, timeSegments);
                            return (
